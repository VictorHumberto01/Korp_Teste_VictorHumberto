...
func ErrorHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if len(c.Errors) > 0 {
			...
			switch {
			case errors.Is(err, domainerrors.ErrProdutoNotFound):
				code = http.StatusNotFound
				message = err.Error()
			case errors.Is(err, domainerrors.ErrCodigoAlreadyExists) ||
				errors.Is(err, domainerrors.ErrConcurrencyConflict) ||
				errors.Is(err, domainerrors.ErrSaldoInsuficiente):
				code = http.StatusConflict
				message = err.Error()
			case errors.Is(err, domainerrors.ErrCodigoInvalido) ||
				errors.Is(err, domainerrors.ErrDescricaoInvalida) ||
...
func formatFieldError(fe validator.FieldError) string {
	...
	case "gte":
		return field + " deve ser maior ou igual a " + fe.Param()
	default:
